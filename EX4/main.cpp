...
bool readFileToMatrix (const std::string &filePath, Matrix &mat)
{
    ...
    // Check if the file size matches the matrix size
    file.seekg(0, std::ios::end);
    std::streampos fileSize = file.tellg();
    std::streampos expectedSize = sizeof(float) * rows * cols;
    file.seekg(0, std::ios::beg);
    if (fileSize != expectedSize)
    {
        std::cout << "File size doesn't match matrix size." << std::endl;
        file.close();
        return false;
    }
    ...
    for (int i = 0; i < rows; ++i)
    {
        ...
        {
            file.read(reinterpret_cast<char*>(&mat[i * cols + j])
                    , sizeof(float));
        }
    }
