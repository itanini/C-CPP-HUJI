...
class User {
    ...
    rank_map _rankings;
    const std::shared_ptr<RecommendationSystem> _recommendationSystem;

public:
    /**
     * Constructor for the class
     */
    User(std::string username, rank_map &map,
         std::shared_ptr<RecommendationSystem> recommendationSystem);
    ...
    std::string &get_name() const {
        return const_cast<std::string &>(_username); }
    ...
    std::unordered_map<sp_movie, double, hash_func, equal_func>
            get_ranks() const {return _rankings;}
     ...
     */
    friend std::ostream &operator<<(std::ostream &o_stream, User &user);
